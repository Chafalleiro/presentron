 ...
 */
// Global image cache to prevent race conditions
                ...
                db.getAll("slides").then(function (datasets) {
                    ...
                    loadSlideImages(anArr, function () {
                        ...
                        const myTimeout = setTimeout(function () {
                            stopLoadingSound(sliTrack);
                        }, 5000);
                        const fanTimeout = setTimeout(function () {
                            stopLoadingSoundAndStartOtherTrack(sliFan, 'snd/ventiladorproyectorvideo.mp3');
                        }, 150);
...
function reposition() {
    ...
    if (sw_on) {
        ...
        // 1. Obtener los datos del registro actual
        const currentData = anArr[sliNdx];
        const imageSrc = currentData["slide"]; // Tu string base64

        // 2. Crear una imagen temporal para pre-cargar/decodificar
        const tempImg = new Image();
        // 3. Definir qué hacer cuando la imagen esté lista
        tempImg.onload = function () {

            // Ahora sí, la imagen está decodificada en memoria

            // Asignar al elemento real
            imgAct.src = imageSrc;
            sldAct.style.backgroundImage = "url('" + imageSrc + "')";
        ...
        // 5. Manejo de errores por si el base64 está corrupto
        tempImg.onerror = function () {
            console.error("Error decoding slide image at index " + sliNdx);
            sldAct.style.visibility = "visible"; // Mostrar al menos el contador o fondo de error
            // Opcional: mostrar un placeholder
            sldAct.style.backgroundImage = "url('img/error_placeholder.png')";
        };

        // 6. Iniciar la carga asignando el src (esto dispara onload/onerror)
...
async function rndRes() {
    ...
    if (anArr[sliNdx]["dirt"]) {
        rndDirt();
        console.log("dirt");
    }
    if (anArr[sliNdx]["damage"]) {
        rndDmg();
        console.log("damage");
    }
    rndBlur()
